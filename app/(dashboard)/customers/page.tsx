...
export default function CustomersPage() {
  ...
  // Feature Gate
  if (!loading && !hasFeature('customer_management')) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-4rem)] p-4 text-center">
        <div className="bg-primary/10 p-4 rounded-full mb-4">
          <Users className="h-12 w-12 text-primary" />
        </div>
        <h1 className="text-2xl font-bold mb-2">Guest CRM Add-on Required</h1>
        <p className="text-muted-foreground max-w-md mb-6">
          The Guest CRM feature allows you to manage customer profiles, track visits, and personalize service. This is a premium add-on.
        </p>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {/* Birthday indicator */}
                          {(() => {
                            const birthdayInfo = getBirthdayInfo(customer.profile?.date_of_birth)
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-600">
                          {(customer.profile?.email || customer.guest_email) && (
                            <a 
                              href={`mailto:${customer.profile?.email || customer.guest_email}`}
                          ...
                          {(customer.profile?.phone_number || customer.guest_phone) && (
                            <a 
                              href={`tel:${customer.profile?.phone_number || customer.guest_phone}`}
                              className="flex items-center gap-1 hover:text-primary"
                              onClick={(e) => e.stopPropagation()}
                            >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right space-y-1">
                        ...
                        {/* Booking breakdown - use calculated values from actual bookings */}
                        <div className="flex items-center gap-2 text-xs flex-wrap">
                          {((customer as any)._calculated_completed || 0) > 0 && (
                            <span className="text-green-600">{(customer as any)._calculated_completed} Completed</span>
                          )}
                          {((customer as any)._calculated_cancelled || 0) > 0 && (
                            <span className="text-orange-600">{(customer as any)._calculated_cancelled} Cancelled</span>
                          )}
                          {((customer as any)._calculated_declined || 0) > 0 && (
                            <span className="text-gray-500">{(customer as any)._calculated_declined} Declined</span>
                          )}
                          {((customer as any)._calculated_no_show || 0) > 0 && (
                            <span className="text-red-600">{(customer as any)._calculated_no_show} No-Show</span>
                          )}
                        </div>
