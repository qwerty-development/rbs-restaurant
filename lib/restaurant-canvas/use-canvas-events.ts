...
interface UseCanvasEventsOptions {
  ...
  updateTransform: (
    transform: Partial<{ position: CanvasPosition; zoom: number }>
  ) => void;
...
export const useCanvasEvents = ({
  ...
  const getMousePosition = useCallback(
    (e: MouseEvent | React.MouseEvent): CanvasPosition => {
  ...
  const getTouchPosition = useCallback(
    (touch: React.Touch | Touch): CanvasPosition => {
      if (!canvasRef.current) return { x: 0, y: 0 };
  ...
  const calculateTouchDistance = useCallback(
    (touch1: CanvasPosition, touch2: CanvasPosition): number => {
  ...
  const handleMouseMove = useCallback(
    ...
    [
      readOnly,
      getMousePosition,
      canvasState.transform.position,
      updateTransform,
    ]
  );
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      const zoomDelta =
        -e.deltaY * RESTAURANT_CANVAS_CONFIG.ZOOM.wheelSensitivity;
      const newZoom = Math.max(
        RESTAURANT_CANVAS_CONFIG.ZOOM.min,
        Math.min(RESTAURANT_CANVAS_CONFIG.ZOOM.max, transform.zoom + zoomDelta)
      );
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (touches.length === 1) {
        ...
        touchStateRef.current.initialDistance = calculateTouchDistance(
          touches[0],
          touches[1]
        );
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (touches.length === 0) {
        ...
        touchStateRef.current.touches = [
          { id: 0, ...getTouchPosition(touches[0]) },
        ];
      }
    },
    [getTouchPosition]
